<div class="admin-box">
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			<thead>
				<tr>
					<?php if ($this->auth->has_permission('Parametre.Settings.Delete') && isset($records) && is_array($records) && count($records)) : ?>
			...
			<tfoot>
				<?php if ($this->auth->has_permission('Parametre.Settings.Delete')) : ?>
				<tr>
					<td colspan="3">
			...
			<?php foreach ($records as $record) : ?>
				...
				<?php if ($this->auth->has_permission('Parametre.Settings.Edit')) : ?>
				<td><?php echo anchor(SITE_AREA .'/settings/parametre/edit/'. $record->param_id, '<i class="icon-pencil">&nbsp;</i>' .  $record->param_libelle) ?></td>
				<?php else: ?>
